...
def log_access(url, action):
    #Essa função registra o acesso a um URL específico, juntamente com a ação realizada (permitido, bloqueado, filtrado ou erro).
    log_entry = {
...
def rewrite_urls(html, base_url): #Recebe o conteúdo HTML e a URL base do site original para reescrever os links corretamente
    parsed_base = urlparse(base_url)#Analisa a URL base usando urlparse, que divide a URL em componentes como esquema (http/https), domínio, caminho, etc.
    scheme = parsed_base.scheme#Extrai o esquema (http ou https) da URL base para garantir que os links reescritos mantenham o mesmo protocolo.
    netloc = parsed_base.netloc#Extrai o domínio (netloc) da URL base para garantir que os links reescritos apontem para o mesmo domínio do site original.

    #Regex para converter links absolutos para passar pelo servidor local (localhost:5000)
    html = re.sub(r'(href|src)=["\']https?://([^"\']+)["\']', r'\1="http://127.0.0.1:5000/\2"', html)
    html = re.sub(r'(href|src)=["\'](//[^"\']+)["\']', fr'\1="http://127.0.0.1:5000/{scheme}:\2"', html)
    html = re.sub(r'(href|src)=["\'](/[^/][^"\']*)["\']', fr'\1="http://127.0.0.1:5000/{scheme}://{netloc}\2"', html)
    return html
...
def proxy(path):
    ...
    full_url = request.full_path.lstrip('/?') #Extrai a URL completa do pedido
    ...
    if not full_url or full_url == '':
        ...
        """, 200 #Retorna status "OK" para a página inicial
    ...
    if domain in blocked_sites:
        ...
        return blocked_page(domain), 403 #e retorna a página de bloqueio personalizada com status 403 (Forbidden).
